...
export default class DateTimeField extends Component {
  ...
  onClick = () => {
    ...
    if (this.props.direction === 'up') {
      ...
      if (offset.top + this.refs.widget.offsetHeight > window.offsetHeight + scrollTop && this.refs.widget.offsetHeight + this.refs.datetimepicker.offsetHeight > offset.top) {
        placePosition = 'top';
      } else {
        placePosition = 'bottom';
      }
  ...
  setSelectedHour = (event) => {
    return this.setState({
      selectedDate: this.state.selectedDate.clone().hour(parseInt(event.target.innerHTML, 10)).minute(this.state.selectedDate.minutes())
    }, () => {
      this.closePicker();
  ...
  setSelectedMinute = (event) => {
    return this.setState({
      selectedDate: this.state.selectedDate.clone().hour(this.state.selectedDate.hours()).minute(parseInt(event.target.innerHTML, 10))
    }, () => {
      this.closePicker();
      this.props.onChange(this.state.selectedDate.format(this.props.format));
      return this.setState({
  ...
  setSelectedDate = (event) => {
    ...
    if (target.className && !target.className.match(/disabled/g)) {
      ...
      return this.setState({
        selectedDate: this.state.viewDate.clone().month(month).date(parseInt(event.target.innerHTML, 10)).hour(this.state.selectedDate.hours()).minute(this.state.selectedDate.minutes())
      }, () => {
        this.closePicker();
        this.props.onChange(this.state.selectedDate.format(this.props.format));
        return this.setState({
    ...
    return (
      <div>
        ...
        <div className={'input-group date ' + this.size()} ref="datetimepicker">
          <input className="form-control" placeholder={this.props.placeholder} onChange={this.onChange} type="text" value={this.state.inputValue} {...this.props.inputProps} />
